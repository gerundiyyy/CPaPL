...
public class FieldManager {
    ...
    public String parseBrand() {
        ...
        if (s.isEmpty()) throw new IllegalArgumentException("Brand: field is necessarily");
        return s;
    }

    public int parseYear() {
        String s = getRawYear().trim();
        if (s.isEmpty()) throw new IllegalArgumentException("Year: field is necessarily");
        try {
            return Integer.parseInt(s);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Year: input integer");
        }
    }

    public double parseVolume() {
        String s = getRawVolume().trim();
        if (s.isEmpty()) throw new IllegalArgumentException("Volume: field is necessarily");
        try {
            return Double.parseDouble(s);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Volume: input number");
        }
    }

    public double parseSpeed() {
        String s = getRawSpeed().trim();
        if (s.isEmpty()) throw new IllegalArgumentException("Speed: field is necessarily");
        try {
            return Double.parseDouble(s);
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Speed: input number");
        }
    }
